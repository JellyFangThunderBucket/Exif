...
export const toolRegistry = {
  ...
  strings: { id: 'strings', displayName: 'strings', command: 'strings', supportedMimeFamilies: ['image', 'video', 'audio', 'application', 'text'], timeoutMs: 10000, maxOutputBytes: outputLimits.stringsMaxBytes, allowedArgs: ['-n'], enabledByDefault: true, expensive: false, mayExtractFiles: false, installHint: 'sudo apt install -y binutils', purpose: 'Printable byte sequence discovery with capped output.' },
  hash: { id: 'hash', displayName: 'Node crypto hashes', command: 'node:crypto', supportedMimeFamilies: ['image', 'video', 'audio', 'application', 'text'], timeoutMs: 5000, maxOutputBytes: 4096, allowedArgs: [], enabledByDefault: true, expensive: false, mayExtractFiles: false, purpose: 'SHA-256 and MD5 file comparison hashes.', installHint: 'Built in to Node.js.' },
  file: { id: 'file', displayName: 'file', command: 'file', supportedMimeFamilies: ['image', 'video', 'audio', 'application', 'text'], timeoutMs: 5000, maxOutputBytes: 16000, allowedArgs: ['--brief', '--mime-type'], enabledByDefault: true, expensive: false, mayExtractFiles: false, purpose: 'Magic-byte file identification.', installHint: 'sudo apt install -y file' },
  zbarimg: { id: 'zbarimg', displayName: 'zbarimg', command: 'zbarimg', supportedMimeFamilies: ['image'], timeoutMs: 15000, maxOutputBytes: 64000, allowedArgs: ['--quiet'], enabledByDefault: true, expensive: false, mayExtractFiles: false, installHint: 'sudo apt install -y zbar-tools', purpose: 'QR and barcode detection.' },
  tesseract: { id: 'tesseract', displayName: 'Tesseract OCR', command: 'tesseract', supportedMimeFamilies: ['image'], timeoutMs: 30000, maxOutputBytes: 128000, allowedArgs: ['stdout', '-l'], enabledByDefault: false, expensive: true, mayExtractFiles: true, installHint: 'sudo apt install -y tesseract-ocr', purpose: 'Explicit OCR text extraction.' },
...
export function verifyHash(actual, expected = '') { const clean = String(expected).replace(/\s+/g, '').toLowerCase(); if (!clean) return { status: 'empty', algorithm: 'Unknown' }; const algorithm = /^[a-f0-9]{32}$/.test(clean) ? 'MD5' : /^[a-f0-9]{64}$/.test(clean) ? 'SHA-256' : 'Unknown'; if (algorithm === 'Unknown') return { status: 'invalid', algorithm }; return { status: String(actual).toLowerCase() === clean ? 'match' : 'mismatch', algorithm }; }
...
export function parseMediaInfoJson(raw = '') {
  ...
  return { container: general.Format || unknown, duration: general.Duration || unknown, fileSize: general.FileSize || unknown, overallBitrate: general.OverallBitRate || unknown, videoCodec: video.Format || unknown, audioCodec: audio.Format || unknown, resolution: video.Width && video.Height ? `${video.Width}x${video.Height}` : unknown, frameRate: video.FrameRate || unknown, pixelFormat: video.ColorSpace || video.ChromaSubsampling || unknown, aspectRatio: video.DisplayAspectRatio || unknown, audioChannels: audio.Channels || unknown, sampleRate: audio.SamplingRate || unknown, encoder: general.Encoded_Application || general.WritingApplication || unknown, creationTime: general.Recorded_Date || general.Encoded_Date || unknown, streams: tracks.map((t, i) => ({ index: i, type: t['@type'] || unknown, codec: t.Format || unknown, language: t.Language || unknown })) };
}
export function parseFfprobeJson(raw = '') {
  const unknown = 'Unknown'; let json = {}; try { json = JSON.parse(raw || '{}'); } catch {}
  const streams = json.streams || [], format = json.format || {};
  const video = streams.find(s => s.codec_type === 'video') || {}, audio = streams.find(s => s.codec_type === 'audio') || {};
  return { container: format.format_name || unknown, duration: format.duration || unknown, fileSize: format.size || unknown, overallBitrate: format.bit_rate || unknown, videoCodec: video.codec_name || unknown, audioCodec: audio.codec_name || unknown, resolution: video.width && video.height ? `${video.width}x${video.height}` : unknown, frameRate: video.avg_frame_rate || unknown, pixelFormat: video.pix_fmt || unknown, aspectRatio: video.display_aspect_ratio || unknown, audioChannels: audio.channels || unknown, sampleRate: audio.sample_rate || unknown, encoder: format.tags?.encoder || unknown, creationTime: format.tags?.creation_time || unknown, streams: streams.map((s, i) => ({ index: s.index ?? i, type: s.codec_type || unknown, codec: s.codec_name || unknown, language: s.tags?.language || unknown })) };
}
export function parseIdentify(raw = '') {
  const get = (re) => (raw.match(re) || [])[1]?.trim() || 'Unknown';
  return { format: get(/Format:\s*([^\n]+)/), dimensions: get(/Geometry:\s*([^\n]+)/), orientation: get(/Orientation:\s*([^\n]+)/), colorspace: get(/Colorspace:\s*([^\n]+)/), bitDepth: get(/Depth:\s*([^\n]+)/), alpha: get(/Alpha:\s*([^\n]+)/), compression: get(/Compression:\s*([^\n]+)/), density: get(/Resolution:\s*([^\n]+)/), profiles: [...raw.matchAll(/Profile-([^:\n]+):/g)].map(m => m[1]), channelInfo: get(/Channel statistics:\s*([\s\S]*?)(?:Image statistics:|Rendering intent:|$)/) };
}
export function parseBinwalk(raw = '') { return String(raw).split('\n').map(l => l.trim()).filter(Boolean).filter(l => !/^DECIMAL\s+HEXADECIMAL/i.test(l)).map(line => { const m = line.match(/^(\d+)\s+0x([0-9A-Fa-f]+)\s+(.+)$/); return m ? { offset: Number(m[1]), hex: `0x${m[2]}`, description: m[3] } : { offset: null, hex: 'Unknown', description: line }; }); }
export function normalizeTimestamp(input) { const s = String(input).trim(); if (!/^(?:\d+(?:\.\d+)?|\d{1,2}:\d{2}(?::\d{2})?(?:\.\d{1,3})?)$/.test(s)) throw new Error('Timestamp format is not allowed.'); const parts = s.split(':').map(Number); const seconds = parts.length === 1 ? Number(s) : parts.length === 2 ? parts[0] * 60 + parts[1] : parts[0] * 3600 + parts[1] * 60 + parts[2]; if (!Number.isFinite(seconds) || seconds < 0) throw new Error('Timestamp is out of range.'); return seconds; }
export function assertTimestampWithinDuration(input, duration) { const seconds = normalizeTimestamp(input); if (Number.isFinite(Number(duration)) && seconds > Number(duration)) throw new Error('Timestamp exceeds video duration.'); return seconds; }
export function reportText(report = {}) { return [`Investigation report for ${report.filename || 'Unknown'}`, `MIME: ${report.mimeType || 'Unknown'}`, `Size: ${report.size ?? 'Unknown'}`, `SHA-256: ${report.hashes?.sha256 || 'Unknown'}`, `MD5: ${report.hashes?.md5 || 'Unknown'}`, `Completed: ${(report.completedAnalyses || []).join(', ') || 'None'}`, `Skipped: ${(report.skippedAnalyses || []).map(s => s.label || s.id).join(', ') || 'None'}`, `Failed: ${(report.failedAnalyses || []).map(f => f.id || f.label).join(', ') || 'None'}`].join('\n'); }
